// UART.cpp : This file contains the 'main' function. Program execution begins and ends there.
//

#include "Serial.h";

#include <windows.h>
#include <stdio.h>
#include <iostream>
#include <vector>
#include <intrin.h>
#include <bitset>

#define COMM_MSG 0x01;
#define ANSW_MSG 0x02;

using namespace std;


void SwapBytes(uint16_t &value) {
    value = (value >> 8) | (value << 8);
}

void SwapBytes(int16_t &value) {
    value = (value >> 8) | (value << 8);
}

void SwapBytes(uint32_t &value) {
    std::uint32_t tmp = ((value << 8) & 0xFF00FF00) | ((value >> 8) & 0xFF00FF);
    value = (tmp << 16) | (tmp >> 16);
}

void SwapBytes(float &f) {
    uint32_t value = 0;
    memcpy(&value, &f, sizeof value);
    std::uint32_t tmp = ((value << 8) & 0xFF00FF00) | ((value >> 8) & 0xFF00FF);
    value = (tmp << 16) | (tmp >> 16);
    memcpy(&f, &value, sizeof f);
}

void SwapBytes(uint64_t &value) {
    value = ((value & 0x00000000FFFFFFFFull) << 32) | ((value & 0xFFFFFFFF00000000ull) >> 32);
    value = ((value & 0x0000FFFF0000FFFFull) << 16) | ((value & 0xFFFF0000FFFF0000ull) >> 16);
    value = ((value & 0x00FF00FF00FF00FFull) << 8) | ((value & 0xFF00FF00FF00FF00ull) >> 8);
}

void SwapBytes(double &d) {
    uint64_t value = 0;
    memcpy(&value, &d, sizeof value);
    value = ((value & 0x00000000FFFFFFFFull) << 32) | ((value & 0xFFFFFFFF00000000ull) >> 32);
    value = ((value & 0x0000FFFF0000FFFFull) << 16) | ((value & 0xFFFF0000FFFF0000ull) >> 16);
    value = ((value & 0x00FF00FF00FF00FFull) << 8) | ((value & 0xFF00FF00FF00FF00ull) >> 8);
    memcpy(&d, &value, sizeof d);
}


class Message {
public:
    uint8_t length;
    uint8_t header;
    uint8_t command;
    uint8_t *command_data;
    uint16_t crc = 0;
    uint8_t* buffer;
    LPCWSTR port;

    Message() {}
    ~Message() {}

    void prt_msg() {
        cout << hex << static_cast<int>(length - 5) << endl;
    }
    
    void create_msg() {
        buffer = new uint8_t[length];
        buffer[0] = length;
        buffer[1] = header;
        buffer[2] = command;
        for (int i = 0; i < length - 5; i++) {
            buffer[i + 3] = command_data[i];
        }
    }
    
    void calculate_crc() {
        uint32_t crc_length = length - 2;

        uint16_t data = 0;

        while (0 != crc_length--) {
            data = *(buffer++) << 8;
            for (uint32_t i = 0; i < 8; i++) {
                if ((data ^ crc) & 0x8000)
                    crc = (crc << 1) ^ 0x1021;
                else
                    crc <<= 1;

                data <<= 1;
            }
        }
        for (size_t i = 0; i < length - 2; i++)
        {
            buffer--;
        }
        uint16_t in = crc;
        uint8_t out[2];
        *(uint16_t*)&out = in;
        buffer[length - 2] = out[1];
        buffer[length - 1] = out[0];
    }

    void send() {
        Serial serial = Serial(port);
        serial.send(buffer, length);
    }
};

class BITCommand : public Message {                                             /////////                 BÝT
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint32_t bitSonucu;
        uint8_t KTSBitSonucu;
        uint32_t TAPABitSonucu;
        uint32_t rezerve1;
        uint32_t rezerve2;
        uint16_t crc;
    };
    
public:
    response_data_struct rds;
    BITCommand() {
        command = 0x11;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;
        cout << "bitSonucu: ";
        SwapBytes(rds.bitSonucu);
        cout << bitset<32>(rds.bitSonucu) << endl;;
        cout << "KTSBitSonucu: ";
        cout << bitset<8>(rds.KTSBitSonucu) << endl;;
        cout << "TAPABitSonucu: ";
        SwapBytes(rds.TAPABitSonucu);
        cout << bitset<32>(rds.TAPABitSonucu) << endl;;
        cout << "Rezerve: ";
        SwapBytes(rds.rezerve1);
        cout << bitset<32>(rds.rezerve1) << endl;;
        cout << "Rezerve: ";
        SwapBytes(rds.rezerve2);
        cout << bitset<32>(rds.rezerve2) << endl;
        cout << "crc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class ICommand : public Message {                                             /////////                 ICommand
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint8_t seriNo[12];
        uint16_t crc;
    };
public:
    response_data_struct rds;
    ICommand() {
        command = 0x21;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "seriNo: ";
        for (size_t i = 0; i < sizeof rds.seriNo; i++)
        {
            cout << rds.seriNo[i];
        }
        

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};


class MCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint8_t islemSonucu;
        uint8_t modDurumu;
        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t modeInfo = 0;
    };

    command_data_struct cds;
    
public:
    response_data_struct rds;
    MCommand() {
        command = 0x20;
        header = COMM_MSG;
        length = 6;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "islemSonucu: ";
        cout << hex << (int)rds.islemSonucu << endl;
        cout << "modDurumu: ";
        cout << hex << (int)rds.modDurumu << endl;


        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class LCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint8_t uky_modu;
        float lp_range;
        double rezerve[3];
        double lp_pos_llh_t[3];
        double rezerve_2;
        double rezerve_3;
        uint8_t rezerve_4;
        uint8_t lp_proxfuze;
        float lp_fts_cond[4];
        float rezerve_5[3];
        double rezerve_6;
        uint16_t lp_proxfuze_dist;
        double rezerve_7;
        uint8_t rezerve_8[11];
        uint16_t crc;
    };
    struct command_data_struct {
        uint8_t uky_modu = 0;
        float lp_range = 0;
        double rezerve[3] = { 0 };
        double lp_pos_llh_t[3] = { 0 };
        double rezerve_2 = 0;
        double rezerve_3 = 0;
        uint8_t rezerve_4 = 0;
        uint8_t lp_proxfuze = 0;
        float lp_fts_cond[4] = { 0 };
        float rezerve_5[3] = { 0 };
        double rezerve_6 = 0;
        uint16_t lp_proxfuze_dist = 0;
        double rezerve_7 = 0;
        uint8_t rezerve_8[11] = { 0 };
    };
    
    command_data_struct cds;
    

public:
    response_data_struct rds;
    LCommand() {
        command = 0x22;
        header = COMM_MSG;
        length = 0x85;
        command_data = new uint8_t[length - 5];
    }

    void bufferCommData() {
        SwapBytes(cds.lp_range);
        for (int i = 0; i < sizeof(cds.rezerve) / 8; i++) {
            SwapBytes(cds.rezerve[i]);
        }
        for (int i = 0; i < sizeof(cds.lp_pos_llh_t) / 8; i++) {
            SwapBytes(cds.lp_pos_llh_t[i]);
        }
        SwapBytes(cds.rezerve_2);
        SwapBytes(cds.rezerve_3);
        for (int i = 0; i < sizeof(cds.lp_fts_cond) / 4; i++) {
            SwapBytes(cds.lp_fts_cond[i]);
        }

        for (int i = 0; i < sizeof(cds.rezerve_5) / 4; i++) {
            SwapBytes(cds.rezerve_5[i]);
        }
        SwapBytes(cds.rezerve_6);
        SwapBytes(cds.lp_proxfuze_dist);
        SwapBytes(cds.rezerve_7);
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "uky_modu: ";
        cout << hex << (int)rds.uky_modu << endl;
        cout << "lp_range: ";
        SwapBytes(rds.lp_range);
        cout << rds.lp_range << endl;
        cout << "Rezerve:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.rezerve[i]);
            cout << "\t" << rds.rezerve[i] << endl;
        }
        cout << "lp_pos_llh_t:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.lp_pos_llh_t[i]);
            cout << "\t" << rds.lp_pos_llh_t[i] << endl;;
        }
        cout << "rezerve_2: ";
        SwapBytes(rds.rezerve_2);
        cout << rds.rezerve_2 << endl;
        cout << "rezerve_3: ";
        SwapBytes(rds.rezerve_3);
        cout << rds.rezerve_3 << endl;
        cout << "rezerve_4: ";
        cout << hex << (int)rds.rezerve_4 << endl;
        cout << "lp_proxfuze: ";
        cout << hex << (int)rds.lp_proxfuze << endl;
        cout << "lp_fts_cond:\n";
        for (size_t i = 0; i < 4; i++)
        {
            SwapBytes(rds.lp_fts_cond[i]);
            cout << "\t" << rds.lp_fts_cond[i] << endl;;
        }
        cout << "rezerve_5:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.rezerve_5[i]);
            cout << "\t" << rds.rezerve_5[i] << endl;
        }
        cout << "rezerve_6: ";
        SwapBytes(rds.rezerve_6);
        cout << rds.rezerve_6 << endl;
        cout << "lp_proxfuze_dist: ";
        SwapBytes(rds.lp_proxfuze_dist);
        printf("%u\n", (uint16_t)rds.lp_proxfuze_dist);
        cout << "rezerve_7: ";
        SwapBytes(rds.rezerve_7);
        cout << rds.rezerve_7 << endl;
        cout << "Rezerve_8: ";
        for (size_t i = 0; i < sizeof rds.rezerve_8; i++)
        {
            cout << rds.rezerve_8[i];
        }

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class GCCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint8_t OUT_GC_CK;
        uint16_t OUT_GC_Diag;
        uint8_t KKS1_UyduSayisi;
        uint8_t KKS2_UyduSayisi;
        uint8_t KKS3_UyduSayisi;
        uint8_t Rezerve1[5];
        double Enlem;
        double Boylam;
        double Yukseklik;
        float Dogu_Hizi;
        float Kuzey_Hizi;
        float Yukari_Hizi;
        double Enlem2;
        double Boylam2;
        double Yukseklik2;
        float Dogu_Hizi2;
        float Kuzey_Hizi2;
        float Yukari_Hizi2;
        double Enlem3;
        double Boylam3;
        double Yukseklik3;
        float Dogu_Hizi3;
        float Kuzey_Hizi3;
        float Yukari_Hizi3;
        uint8_t Rezerve2[83];
        uint16_t crc;
    };
public:
    response_data_struct rds;
    GCCommand() {
        command = 0x23;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "OUT_GC_CK: ";
        cout << hex << (int)rds.OUT_GC_CK << endl;
        cout << "OUT_GC_Diag: ";
        SwapBytes(rds.OUT_GC_Diag);
        cout << bitset<16>(rds.OUT_GC_Diag) << endl;
        cout << "KKS1_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKS1_UyduSayisi);
        cout << "KKS2_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKS2_UyduSayisi);
        cout << "KKS3_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKS3_UyduSayisi);
        cout << "Rezerve: ";
        for (size_t i = 0; i < 5; i++)
        {
            cout << rds.Rezerve1[i];
        }
        cout << "Enlem: ";
        SwapBytes(rds.Enlem);
        cout << rds.Enlem << endl;
        cout << "Boylam: ";
        SwapBytes(rds.Boylam);
        cout << rds.Boylam << endl;
        cout << "Yukseklik: ";
        SwapBytes(rds.Yukseklik);
        cout << rds.Yukseklik << endl; 
        cout << "Dogu_Hizi: ";
        SwapBytes(rds.Dogu_Hizi);
        cout << rds.Dogu_Hizi << endl;
        cout << "Kuzey_Hizi: ";
        SwapBytes(rds.Kuzey_Hizi);
        cout << rds.Kuzey_Hizi << endl;
        cout << "Yukari_Hizi: ";
        SwapBytes(rds.Yukari_Hizi);
        cout << rds.Yukari_Hizi << endl;
        cout << "Enlem2: ";
        SwapBytes(rds.Enlem2);
        cout << rds.Enlem2 << endl;
        cout << "Boylam2: ";
        SwapBytes(rds.Boylam2);
        cout << rds.Boylam2 << endl;
        cout << "Yukseklik2: ";
        SwapBytes(rds.Yukseklik2);
        cout << rds.Yukseklik2 << endl; 
        cout << "Dogu_Hizi2: ";
        SwapBytes(rds.Dogu_Hizi2);
        cout << rds.Dogu_Hizi2 << endl;
        cout << "Kuzey_Hizi2: ";
        SwapBytes(rds.Kuzey_Hizi2);
        cout << rds.Kuzey_Hizi2 << endl;
        cout << "Yukari_Hizi2: ";
        SwapBytes(rds.Yukari_Hizi2);
        cout << rds.Yukari_Hizi2 << endl;
        cout << "Enlem3: ";
        SwapBytes(rds.Enlem3);
        cout << rds.Enlem3 << endl;
        cout << "Boylam3: ";
        SwapBytes(rds.Boylam3);
        cout << rds.Boylam3 << endl;
        cout << "Yukseklik3: ";
        SwapBytes(rds.Yukseklik3);
        cout << rds.Yukseklik3 << endl; 
        cout << "Dogu_Hizi3: ";
        SwapBytes(rds.Dogu_Hizi3);
        cout << rds.Dogu_Hizi3 << endl;
        cout << "Kuzey_Hizi3: ";
        SwapBytes(rds.Kuzey_Hizi3);
        cout << rds.Kuzey_Hizi3 << endl;
        cout << "Yukari_Hizi3: ";
        SwapBytes(rds.Yukari_Hizi3);
        cout << rds.Yukari_Hizi3 << endl;




        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class FCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
        uint8_t OUT_F_CK;
        uint32_t OUT_F_Diag;
        uint8_t tfrBasarim;
        float eulerF[3];
        uint8_t KKSUyduSayýsý1;
        uint8_t KKSUyduSayýsý2;
        uint8_t KKSUyduSayýsý3;
        uint8_t KTSMotorGerilimDurumu;
        uint16_t HariciVoltaj;
        uint16_t DahiliVoltaj;
        uint16_t KTSVoltaj;
        float WF[3];
        float AF[3];
        float VF[3];
        float XF[3];
        float XF_1[3];
        float XF_2[3];
        float XF_3[3];
        uint8_t Rezerve[47];
        uint16_t crc;



    };
    struct command_data_struct {
        uint16_t validityWord = 0;
        uint16_t timeTagFXR = 0;
        float latitudeHV = 0;
        float longitudeHV = 0;
        float baroAltitudeHV = 0;
        float velocityXHV = 0;
        float velocityEastHV = 0;
        float velocityDownHV = 0;
        int16_t rollAngleHV = 0;
        int16_t pitchAngleHV = 0;
        int16_t headingHV = 0;
        float wanderAngleHV = 0;
        int16_t velocityWindXHV = 0;
        int16_t velocityWindYHV = 0;
        int16_t velocityWindZHV = 0;
        uint16_t pressureStaticHV = 0;
        int16_t temperatureStaticHV = 0;
        int16_t trueAirSpeedHV = 0;
        int16_t positionDeltaXLAU = 0;
        int16_t positionDeltaYLAU = 0;
        int16_t positionDeltaZLAU = 0;
        float lauRollAngle = 0;
        float lauIncidence = 0;
        float lauMisalignment = 0;
    };

    command_data_struct cds;

public:
    response_data_struct rds;
    FCommand() {
        command = 0x24;
        header = COMM_MSG;
        length = 73;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        SwapBytes(cds.validityWord);
        SwapBytes(cds.timeTagFXR);
        SwapBytes(cds.latitudeHV);
        SwapBytes(cds.longitudeHV);
        SwapBytes(cds.baroAltitudeHV);
        SwapBytes(cds.velocityXHV);
        SwapBytes(cds.velocityXHV);
        SwapBytes(cds.velocityEastHV);
        SwapBytes(cds.velocityDownHV);
        SwapBytes(cds.rollAngleHV);
        SwapBytes(cds.pitchAngleHV);
        SwapBytes(cds.headingHV);
        SwapBytes(cds.wanderAngleHV);
        SwapBytes(cds.velocityWindXHV);
        SwapBytes(cds.velocityWindYHV);
        SwapBytes(cds.velocityWindZHV);
        SwapBytes(cds.pressureStaticHV);
        SwapBytes(cds.temperatureStaticHV);
        SwapBytes(cds.trueAirSpeedHV);
        SwapBytes(cds.positionDeltaXLAU);
        SwapBytes(cds.positionDeltaYLAU);
        SwapBytes(cds.positionDeltaZLAU);
        SwapBytes(cds.lauRollAngle);
        SwapBytes(cds.lauIncidence);
        SwapBytes(cds.lauMisalignment);
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "OUT_F_CK: ";
        cout << hex << (int)rds.OUT_F_CK << endl;
        cout << "OUT_F_Diag: ";
        SwapBytes(rds.OUT_F_Diag);
        cout << bitset<32>(rds.OUT_F_Diag) << endl;
        cout << "tfrBasarim: ";
        cout << hex << (int)rds.tfrBasarim << endl;
        cout << "eulerF:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.eulerF[i]);
            cout << "\t" << rds.eulerF[i] << endl;
        }
        cout << "KKS1_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKSUyduSayýsý1);
        cout << "KKS2_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKSUyduSayýsý2);
        cout << "KKS3_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKSUyduSayýsý3);
        cout << "KTSMotorGerilimDurumu: ";
        cout << bitset<8>(rds.KTSMotorGerilimDurumu) << endl;
        cout << "HariciVoltaj: ";
        SwapBytes(rds.HariciVoltaj);
        printf("%u\n", (uint16_t)rds.HariciVoltaj);
        cout << "DahiliVoltaj: ";
        SwapBytes(rds.DahiliVoltaj);
        printf("%u\n", (uint16_t)rds.DahiliVoltaj);
        cout << "KTSVoltaj: ";
        SwapBytes(rds.KTSVoltaj);
        printf("%u\n", (uint16_t)rds.KTSVoltaj);
        cout << "WF:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.WF[i]);
            cout << "\t" << rds.WF[i] << endl;;
        }
        cout << "AF:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.AF[i]);
            cout << "\t" << rds.AF[i] << endl;;
        }
        cout << "VF:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.VF[i]);
            cout << "\t" << rds.VF[i] << endl;;
        }
        cout << "XF:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.XF[i]);
            cout << "\t" << rds.XF[i] << endl;;
        }
        cout << "XF_1:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.XF_1[i]);
            cout << "\t" << rds.XF_1[i] << endl;;
        }
        cout << "XF_2:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.XF_2[i]);
            cout << "\t" << rds.XF_2[i] << endl;;
        }
        cout << "XF_3:\n";
        for (size_t i = 0; i < 3; i++)
        {
            SwapBytes(rds.XF_3[i]);
            cout << "\t" << rds.XF_3[i] << endl;;
        }

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class ACommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;
 
        uint8_t KKSUyduSaysisi1;
        double LATITUDE1;                               
        double LONGITUDE1;
        double ALTITUDE1;
        double X_VEL1;
        double Y_VEL1;
        double Z_VEL1;
        float gyrX;
        float gyrY;
        float gyrZ;
        float accX;
        float accY;
        float accZ;
        float IA1;
        float IA2;
        float IA3;
        float IG1;
        float IG2;
        float IG3;
        uint8_t KKSUyduSaysisi2;
        double Vx;
        double Vy;
        double Vz;
        double LATITUDE2;
        double LONGITUDE2;
        double ALTITUDE2;
        uint8_t KKS3_UyduSayisi;
        double XKonum3;
        double YKonum3;
        double ZKonum3;
        double XHizi3;
        double YHizi3;
        double ZHizi3;

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint16_t Sure = 4;
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    ACommand() {
        command = 0x26;
        header = COMM_MSG;
        length = 0x07;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        SwapBytes(cds.Sure);
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "KKS1_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKSUyduSaysisi1);
        
        cout << "LATITUDE1: ";
        SwapBytes(rds.LATITUDE1);       
        cout << rds.LATITUDE1 << endl;
        cout << "LONGITUDE1: ";
        SwapBytes(rds.LONGITUDE1);
        cout << rds.LONGITUDE1 << endl;
        cout << "ALTITUDE1: ";
        SwapBytes(rds.ALTITUDE1);
        cout << rds.ALTITUDE1 << endl;
        cout << "X_VEL1: ";
        SwapBytes(rds.X_VEL1);
        cout << rds.X_VEL1 << endl;
        cout << "Y_VEL1: ";
        SwapBytes(rds.Y_VEL1);
        cout << rds.Y_VEL1 << endl;
        cout << "Z_VEL1: ";
        SwapBytes(rds.Z_VEL1);
        cout << rds.Z_VEL1 << endl;
        cout << "gyrX: ";
        SwapBytes(rds.gyrX);
        cout << rds.gyrX << endl;
        cout << "gyrY: ";
        SwapBytes(rds.gyrY);
        cout << rds.gyrY << endl;
        cout << "gyrZ: ";
        SwapBytes(rds.gyrZ);
        cout << rds.gyrZ << endl;
        cout << "accX: ";
        SwapBytes(rds.accX);
        cout << rds.accX << endl;
        cout << "accY: ";
        SwapBytes(rds.accY);
        cout << rds.accY << endl;
        cout << "accZ: ";
        SwapBytes(rds.accZ);
        cout << rds.accZ << endl;
        cout << "IA1: ";
        SwapBytes(rds.IA1);
        cout << rds.IA1 << endl;
        cout << "IA2: ";
        SwapBytes(rds.IA2);
        cout << rds.IA2 << endl;
        cout << "IA3: ";
        SwapBytes(rds.IA3);
        cout << rds.IA3 << endl;
        cout << "IG1: ";
        SwapBytes(rds.IG1);
        cout << rds.IG1 << endl;
        cout << "IG2: ";
        SwapBytes(rds.IG2);
        cout << rds.IG2 << endl;
        cout << "IG3: ";
        SwapBytes(rds.IG3);
        cout << rds.IG3 << endl;

        cout << "KKS2_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKSUyduSaysisi2);

        cout << "Vx: ";
        SwapBytes(rds.Vx);
        cout << rds.Vx << endl;
        cout << "Vy: ";
        SwapBytes(rds.Vy);
        cout << rds.Vy << endl;
        cout << "Vz: ";
        SwapBytes(rds.Vz);
        cout << rds.Vz << endl;
        cout << "LATITUDE2: ";
        SwapBytes(rds.LATITUDE2);
        cout << rds.LATITUDE2 << endl;
        cout << "LONGITUDE2: ";
        SwapBytes(rds.LONGITUDE2);
        cout << rds.LONGITUDE2 << endl;
        cout << "ALTITUDE2: ";
        SwapBytes(rds.ALTITUDE2);
        cout << rds.ALTITUDE2 << endl;

        cout << "KKS3_UyduSayisi: ";
        printf("%u\n", (uint8_t)rds.KKS3_UyduSayisi);

        cout << "XKonum3: ";
        SwapBytes(rds.XKonum3);
        cout << rds.XKonum3 << endl;
        cout << "YKonum3: ";
        SwapBytes(rds.YKonum3);
        cout << rds.YKonum3 << endl;
        cout << "ZKonum3: ";
        SwapBytes(rds.ZKonum3);
        cout << rds.ZKonum3 << endl;
        cout << "XHizi3: ";
        SwapBytes(rds.XHizi3);
        cout << rds.XHizi3 << endl;
        cout << "YHizi3: ";
        SwapBytes(rds.YHizi3);
        cout << rds.YHizi3 << endl;
        cout << "ZHizi3: ";
        SwapBytes(rds.ZHizi3);
        cout << rds.ZHizi3 << endl;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class OCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t haberlesmeDurumu;
        uint8_t KTS1TakasCVerisi[34];
        uint8_t KTS2TakasCVerisi[34];
        uint8_t KTS3TakasCVerisi[34];
        uint8_t KTS4TakasCVerisi[34];

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint32_t CASCommand[4] = {1, 2, 3, 4};
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    OCommand() {
        command = 0x34;
        header = COMM_MSG;
        length = 21;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        for (int i = 0; i < sizeof(cds.CASCommand) / 4; i++) {
            SwapBytes(cds.CASCommand[i]);
        }
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "haberlesmeDurumu: ";
        cout << bitset<8>(rds.haberlesmeDurumu) << endl;

        cout << "KTS1TakasCVerisi: ";
        for (size_t i = 0; i < 34; i++)
        {
            cout << hex << (int)rds.KTS1TakasCVerisi[i];
            cout << " ";
        }
        cout << endl;
        cout << "KTS2TakasCVerisi: ";
        for (size_t i = 0; i < 34; i++)
        {
            cout << hex << (int)rds.KTS2TakasCVerisi[i];
            cout << " ";
        }
        cout << endl;
        cout << "KTS3TakasCVerisi: ";
        for (size_t i = 0; i < 34; i++)
        {
            cout << hex << (int)rds.KTS3TakasCVerisi[i];
            cout << " ";
        }
        cout << endl;
        cout << "KTS3TakasCVerisi: ";
        for (size_t i = 0; i < 34; i++)
        {
            cout << hex << (int)rds.KTS3TakasCVerisi[i];
            cout << " ";
        }

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class OperationTime : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint32_t toplamGucVerilmeSayisi;
        uint32_t toplamGucVerilmeSuresi;
        uint32_t sonCalismaSuresi;

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint32_t sureAyar = 4;
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    OperationTime() {
        command = 0x50;
        header = COMM_MSG;
        length = 0x09;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        SwapBytes(cds.sureAyar);
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "toplamGucVerilmeSayisi: ";
        printf("%u\n", (uint32_t)rds.toplamGucVerilmeSayisi);
        cout << "toplamGucVerilmeSuresi: ";
        printf("%u\n", (uint32_t)rds.toplamGucVerilmeSuresi);
        cout << "sonCalismaSuresi: ";
        printf("%u\n", (uint32_t)rds.sonCalismaSuresi);

        cout << "crc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class Update : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t guncellemeDurum;
        uint8_t guncelVeriNo;
        uint8_t guncelVeriDegeri[8];

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t islemIstegi = 0x0c;
        uint16_t veriTabaniNo = 0x0a0b;
        uint8_t veriDegeri[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    };

    command_data_struct cds;

public:
    response_data_struct rds;
    Update() {
        command = 0x16;
        header = COMM_MSG;
        length = 0x10;
        command_data = new uint8_t[length - 5];
        
    }
    void bufferCommData() {
        SwapBytes(cds.veriTabaniNo);
        memcpy(command_data, (uint8_t *)&cds, sizeof(cds));  
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "guncellemeDurum: ";
        cout << hex << (int)rds.guncellemeDurum << endl;
        cout << "guncelVeriNo: ";
        printf("%u\n", (uint16_t)rds.guncelVeriNo);
        cout << "guncelVeriDegeri: ";
        for (size_t i = 0; i < 8; i++)
        {
            cout << hex << (int)rds.guncelVeriDegeri[i];
            cout << " ";
        }

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class Version : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t versiyonFBY[23];
        uint8_t versiyonFBYAlgDB[23];
        uint8_t versiyonFBYConfigDBY[23];
        uint8_t versiyonFBB[23];
        uint8_t versiyonMosaicD[15];
        uint8_t versiyonMosaicY[15];
        uint8_t versiyonAntaresD[15];
        uint8_t versiyonAntaresY[15];
        uint8_t versiyonAOBD[15];
        uint8_t versiyonAOBY[15];
        uint8_t versiyonFMSKD[15];
        uint8_t versiyonFMSKY[15];

        uint16_t crc;
    };
public:
    response_data_struct rds;
    Version() {
        command = 0x13;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "versiyonFBY: ";
        for (size_t i = 0; i < 23; i++)
        {
            cout<<  rds.versiyonFBY[i];
        }
        cout << endl;
        cout << "versiyonFBYAlgDB: ";
        for (size_t i = 0; i < 23; i++)
        {
            cout << rds.versiyonFBYAlgDB[i];
        }
        cout << endl;
        cout << "versiyonFBYConfigDBY: ";
        for (size_t i = 0; i < 23; i++)
        {
            cout<<  rds.versiyonFBYConfigDBY[i];
        }
        cout << endl;
        cout << "versiyonFBB: ";
        for (size_t i = 0; i < 23; i++)
        {
            cout << rds.versiyonFBB[i];
        }
        cout << endl;
        cout << "versiyonMosaicD: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout<<  rds.versiyonMosaicD[i];
        }
        cout << endl;
        cout << "versiyonMosaicY: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonMosaicY[i];
        }
        cout << endl;
        cout << "versiyonAntaresD: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonAntaresD[i];
        }
        cout << endl;
        cout << "versiyonAntaresY: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonAntaresY[i];
        }
        cout << endl;
        cout << "versiyonAOBD: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonAOBD[i];
        }
        cout << endl;
        cout << "versiyonAOBY: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonAOBY[i];
        }
        cout << endl;
        cout << "versiyonFMSKD: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonFMSKD[i];
        }
        cout << endl;
        cout << "versiyonFMSKY: ";
        for (size_t i = 0; i < 15; i++)
        {
            cout << rds.versiyonFMSKY[i];
        }
        cout << endl;


        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class VoltajSorgu : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint16_t Haricivolt;
        uint16_t Dahilivolt;
        uint16_t KKS1volt;
        uint16_t KKS2volt;
        uint16_t KTSvolt;
        uint16_t AOBvolt;
        uint16_t Yedekvolt;
        uint16_t GBvolt;
        uint16_t GBakim;
        uint16_t FMSKVolt;
        uint16_t FRENVolt;

        uint16_t crc;
    };
public:
    response_data_struct rds;
    VoltajSorgu() {
        command = 0x18;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "Haricivolt: ";
        SwapBytes(rds.Haricivolt);
        cout << rds.Haricivolt;
        cout << "Dahilivolt: ";
        SwapBytes(rds.Dahilivolt);
        cout << rds.Dahilivolt;
        cout << "KKS1volt: ";
        SwapBytes(rds.KKS1volt);
        cout << rds.KKS1volt;
        cout << "KKS2volt: ";
        SwapBytes(rds.KKS2volt);
        cout << rds.KKS2volt;
        cout << "KTSvolt: ";
        SwapBytes(rds.KTSvolt);
        cout << rds.KTSvolt;
        cout << "AOBvolt: ";
        SwapBytes(rds.AOBvolt);
        cout << rds.AOBvolt;
        cout << "Yedekvolt: ";
        SwapBytes(rds.Yedekvolt);
        cout << rds.Yedekvolt;
        cout << "GBvolt: ";
        SwapBytes(rds.GBvolt);  
        cout << rds.GBvolt;
        cout << "GBakim: ";
        SwapBytes(rds.GBakim);
        cout << rds.GBakim;
        cout << "FMSKVolt: ";
        SwapBytes(rds.FMSKVolt);
        cout << rds.FMSKVolt;
        cout << "FRENVolt: ";
        SwapBytes(rds.FRENVolt);
        cout << rds.FRENVolt;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class Reset : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t islemSonucu;

        uint16_t crc;
    };
public:
    response_data_struct rds;
    Reset() {
        command = 0x12;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "islemSonucu: ";
        cout << hex << (int)rds.islemSonucu << endl;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class PCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        float gyrX;
        float gyrY;
        float gyrZ;
        float accX;
        float accY;
        float accZ;
        float IA1;
        float IA2;
        float IA3;
        float IG1;
        float IG2;
        float IG3;

        uint16_t crc;
    };
public:
    response_data_struct rds;
    PCommand() {
        command = 0x31;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "gyrX: ";
        SwapBytes(rds.gyrX);
        cout << rds.gyrX;
        cout << "gyrY: ";
        SwapBytes(rds.gyrY);
        cout << rds.gyrY;
        cout << "gyrZ: ";
        SwapBytes(rds.gyrZ);
        cout << rds.gyrZ;
        cout << "accX: ";
        SwapBytes(rds.accX);
        cout << rds.accX;
        cout << "accY: ";
        SwapBytes(rds.accY);
        cout << rds.accY;
        cout << "accZ: ";
        SwapBytes(rds.accZ);
        cout << rds.accZ;
        cout << "IA1: ";
        SwapBytes(rds.IA1);
        cout << rds.IA1;
        cout << "IA2: ";
        SwapBytes(rds.IA2);
        cout << rds.IA2;
        cout << "IA3: ";
        SwapBytes(rds.IA3);
        cout << rds.IA3;
        cout << "IG1: ";
        SwapBytes(rds.IG1);
        cout << rds.IG1;
        cout << "IG2: ";
        SwapBytes(rds.IG2);
        cout << rds.IG2;
        cout << "IG3: ";
        SwapBytes(rds.IG3);
        cout << rds.IG3;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class SCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t Sonuc;

        uint16_t crc;
    };
public:
    response_data_struct rds;
    SCommand() {
        command = 0x32;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "islemSonucu: ";
        cout << hex << (int)rds.Sonuc << endl;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class FrenCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t islemSonucu;

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t sureAyar = 4;
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    FrenCommand() {
        command = 0x35;
        header = COMM_MSG;
        length = 0x06;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "islemSonucu: ";
        cout << hex << (int)rds.islemSonucu << endl;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class VTReset : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t islemSonucu;

        uint16_t crc;
    };
public:
    response_data_struct rds;
    VTReset() {
        command = 0x66;
        header = COMM_MSG;
        length = 0x05;
        command_data = new uint8_t[length - 5];
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "islemSonucu: ";
        cout << hex << (int)rds.islemSonucu << endl;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class TVTYazma : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t islemIstegi = 0;
        uint16_t veriTabaniNo = 0;
        uint32_t veriTabaniDegeri = 0;
    };

    command_data_struct cds;

public:
    response_data_struct rds;
    TVTYazma() {
        command = 0x16;
        header = COMM_MSG;
        length = 12;
        command_data = new uint8_t[length - 5];

    }
    void bufferCommData() {
        SwapBytes(cds.veriTabaniNo);
        SwapBytes(cds.veriTabaniDegeri);
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class LGCommand : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t LimitValue[235];

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t LimitValue[235] = {0};
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    LGCommand() {
        command = 0x51;
        header = COMM_MSG;
        length = 240;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        for (size_t i = 0; i < 235; i++)
        {
            cout << rds.LimitValue[i];
        }

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class PBITMessage : public Message {
#pragma pack(1)
    struct response_data_struct {
        uint8_t length;
        uint8_t header;
        uint8_t command;

        uint8_t aktivasyoDirenciOkumaDurumu;
        uint8_t direncOkumaRedSebebi;
        uint8_t aktivasyonDirenci;
        uint8_t gobekBagiDurumu;

        uint16_t crc;
    };
    struct command_data_struct
    {
        uint8_t PirotenikSorgu = 0;
    };

    command_data_struct cds;
public:
    response_data_struct rds;
    PBITMessage() {
        command = 0x52;
        header = COMM_MSG;
        length = 6;
        command_data = new uint8_t[length - 5];
    }
    void bufferCommData() {
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
    void print_rds() {
        cout << "length: ";
        cout << hex << (int)rds.length << endl;
        cout << "header: ";
        cout << hex << (int)rds.header << endl;
        cout << "command: ";
        cout << hex << (int)rds.command << endl;

        cout << "aktivasyoDirenciOkumaDurumu: ";
        cout << hex << (int)rds.aktivasyoDirenciOkumaDurumu << endl;

        cout << "haberlesmeDurumu: ";
        cout << bitset<8>(rds.direncOkumaRedSebebi) << endl;

        cout << "guncelVeriNo: ";
        printf("%u\n", (uint8_t)rds.aktivasyonDirenci);

        cout << "gobekBagiDurumu: ";
        cout << hex << (int)rds.gobekBagiDurumu;

        cout << "\ncrc: ";
        SwapBytes(rds.crc);
        cout << hex << (int)rds.crc << endl;
    }
};

class Deneme : public Message {
#pragma pack(1)
    struct command_data_struct
    {
        float deneme1[3] = {1, 1, 1};
        double deneme2[3] = {2, 2, 2};
    };

    command_data_struct cds;

public:
    Deneme() {
        command = 0x31;
        header = COMM_MSG;
        length = 41;
        command_data = new uint8_t[length - 5];

    }
    void bufferCommData() {
        for (int i = 0; i < sizeof(cds.deneme1)/ 4; i++) {
            SwapBytes(cds.deneme1[i]);
        }
        for (int i = 0; i < sizeof(cds.deneme2)/ 8; i++) {
            SwapBytes(cds.deneme2[i]);
        }
        memcpy(command_data, (uint8_t*)&cds, sizeof(cds));
    }
};

                                                                                
int main()
{
    int port_choice;

    cout << "Choose a port:\n\n";
    cout << "1.COM1 \n";
    cout << "2.COM2 \n";

    cin >> port_choice;

    LPCWSTR port_input = L"COM1";

    if (port_choice == 1) {
        port_input = L"COM1";
    }
    else if(port_choice == 2) {
        port_input = L"COM2";
    }

    Serial serial = Serial(port_input);

    int input = -1;
    while (input != 0) {
        cout << "Select a message:\n\n";
        cout << "1.BIT Komutu \n";
        cout << "2.IKomutu \n";
        cout << "3.MKomutu\n";
        cout << "4.LKomutu\n";
        cout << "5.GCKomutu\n";
        cout << "6.FKomutu\n";
        cout << "7.AKomutu\n";
        cout << "8.OKomutu\n";
        cout << "9.OperasyonSuresi\n";
        cout << "10.Guncelle\n";
        cout << "11.Versiyon\n";
        cout << "12.VoltajSorgu\n";
        cout << "13.Reset\n";
        cout << "14.PKomutu\n";
        cout << "15.SKomutu \n";
        cout << "16.FrenKomutu\n";
        cout << "17.VTReset\n";
        cout << "18.TVTYazma\n";
        cout << "19.LG_Komutu\n";
        cout << "20.Piroteknik BIT\n";

        cin >> input;

        switch (input)
        {
        case 1: {
            BITCommand b_msg = BITCommand();
            cout << "Creating BITCommand...\n";
            b_msg.create_msg();
            b_msg.calculate_crc();
            cout << "Sending BITCommand...\n";
            serial.send(b_msg.buffer, b_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&b_msg.rds, recv_msg, len);
            b_msg.print_rds();
            break;
        } 
        case 2: {
            ICommand i_msg = ICommand();
            cout << "Creating ICommand...\n";
            i_msg.create_msg();
            i_msg.calculate_crc();
            cout << "Sending ICommand...\n";
            serial.send(i_msg.buffer, i_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&i_msg.rds, recv_msg, len);
            i_msg.print_rds();
            break;
        }
            
        case 3: {
            cout << "Creating MCommand...\n";
            MCommand m_msg = MCommand();
            m_msg.bufferCommData();
            m_msg.create_msg();
            m_msg.calculate_crc();
            cout << "Sending MCommand...\n";
            serial.send(m_msg.buffer, m_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&m_msg.rds, recv_msg, len);
            m_msg.print_rds();
            break;
        }
            
        case 4: {
            cout << "Creating LCommand...\n";
            LCommand l_msg = LCommand();
            l_msg.bufferCommData();
            l_msg.create_msg();
            l_msg.calculate_crc();
            cout << "Sending LCommand...\n";
            serial.send(l_msg.buffer, l_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&l_msg.rds, recv_msg, len);
            l_msg.print_rds();
            break;
        }
            
        case 5: {
            GCCommand gc_msg = GCCommand();
            cout << "Creating GCCommand...\n";
            gc_msg.create_msg();
            gc_msg.calculate_crc();
            cout << "Sending GCCommand...\n";
            serial.send(gc_msg.buffer, gc_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&gc_msg.rds, recv_msg, len);
            gc_msg.print_rds();
            break;
        }
            
        case 6: {
            FCommand f_msg = FCommand();
            cout << "Creating FCommand...\n";
            f_msg.bufferCommData();
            f_msg.create_msg();
            f_msg.calculate_crc();
            cout << "Sending FCommand...\n";
            serial.send(f_msg.buffer, f_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&f_msg.rds, recv_msg, len);
            f_msg.print_rds();
            break;
        }
            
        case 7: {
            ACommand a_msg = ACommand();
            cout << "Creating ACommand...\n";
            a_msg.bufferCommData();
            a_msg.create_msg();
            a_msg.calculate_crc();
            cout << "Sending ACommand...\n";
            serial.send(a_msg.buffer, a_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&a_msg.rds, recv_msg, len);
            a_msg.print_rds();
            break;
        }
            
        case 8: {
            OCommand o_msg = OCommand();
            cout << "Creating OCommand...\n";
            o_msg.bufferCommData();
            o_msg.create_msg();
            o_msg.calculate_crc();
            cout << "Sending OCommand...\n";
            serial.send(o_msg.buffer, o_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&o_msg.rds, recv_msg, len);
            o_msg.print_rds();
            break;
        }
            
        case 9: {
            OperationTime ot_msg = OperationTime();
            cout << "Creating OperationTime Message...\n";
            ot_msg.bufferCommData();
            ot_msg.create_msg();
            ot_msg.calculate_crc();
            cout << "Sending OperationTime Message...\n";
            serial.send(ot_msg.buffer, ot_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&ot_msg.rds, recv_msg, len);
            ot_msg.print_rds();
            break;
        }
            
        case 10: {
            Update up_msg = Update();
            cout << "Creating UpdateMsg...\n";
            up_msg.bufferCommData();
            up_msg.create_msg();
            up_msg.calculate_crc();
            cout << "Sending UpdateMsg...\n";
            serial.send(up_msg.buffer, up_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&up_msg.rds, recv_msg, len);
            up_msg.print_rds();
            break;
        }
            
        case 11: {
            Version v_msg = Version();
            cout << "Creating VersionMsg...\n";
            v_msg.create_msg();
            v_msg.calculate_crc();
            cout << "Sending VersionMsg...\n";
            serial.send(v_msg.buffer, v_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&v_msg.rds, recv_msg, len);
            v_msg.print_rds();
            break;
        }
            
        case 12: {
            VoltajSorgu volt_msg = VoltajSorgu();
            cout << "Creating VoltajSorgu...\n";
            volt_msg.create_msg();
            volt_msg.calculate_crc();
            cout << "Sending VoltajSorgu...\n";
            serial.send(volt_msg.buffer, volt_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&volt_msg.rds, recv_msg, len);
            volt_msg.print_rds();
            break;
        }
            
        case 13: {
            Reset r_msg = Reset();
            cout << "Creating ResetCommand...\n";
            r_msg.create_msg();
            r_msg.calculate_crc();
            cout << "Sending ResetCommand...\n";
            serial.send(r_msg.buffer, r_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&r_msg.rds, recv_msg, len);
            r_msg.print_rds();
            break;
        }
            
        case 14: {
            PCommand p_msg = PCommand();
            cout << "Creating PCommand...\n";
            p_msg.create_msg();
            p_msg.calculate_crc();
            serial.send(p_msg.buffer, p_msg.length);
            cout << "Sending PCommand...\n";
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&p_msg.rds, recv_msg, len);
            p_msg.print_rds();
            break;
        }
            
        case 15: {
            SCommand s_msg = SCommand();
            cout << "Creating SCommand...\n";
            s_msg.create_msg();
            s_msg.calculate_crc();
            serial.send(s_msg.buffer, s_msg.length);
            cout << "Sending SCommand...\n";
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&s_msg.rds, recv_msg, len);
            s_msg.print_rds();
            break;
        }
            
        case 16: {
            FrenCommand fr_msg = FrenCommand();
            cout << "Creating FrenCommand...\n";
            fr_msg.bufferCommData();
            fr_msg.create_msg();
            fr_msg.calculate_crc();
            cout << "Sending FrenCommand...\n";
            serial.send(fr_msg.buffer, fr_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&fr_msg.rds, recv_msg, len);
            fr_msg.print_rds();
            break;
        }
            
        case 17: {
            VTReset vtr_msg = VTReset();
            cout << "Creating VTResetCommand...\n";
            vtr_msg.create_msg();
            vtr_msg.calculate_crc();
            cout << "Sending VTResetCommand...\n";
            serial.send(vtr_msg.buffer, vtr_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&vtr_msg.rds, recv_msg, len);
            vtr_msg.print_rds();
            break;
        }
            
        case 18: {
            TVTYazma tvt_msg = TVTYazma();
            cout << "Creating TVTYazmaCommand...\n";
            tvt_msg.bufferCommData();
            tvt_msg.create_msg();
            tvt_msg.calculate_crc();
            cout << "Sending TVTYazmaCommand...\n";
            serial.send(tvt_msg.buffer, tvt_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&tvt_msg.rds, recv_msg, len);
            tvt_msg.print_rds();
            break;
        }
            
        case 19: {
            LGCommand lg_msg = LGCommand();
            cout << "Creating LGCommand...\n";
            lg_msg.bufferCommData();
            lg_msg.create_msg();
            lg_msg.calculate_crc();
            cout << "Sending LGCommand...\n";
            serial.send(lg_msg.buffer, lg_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&lg_msg.rds, recv_msg, len);
            lg_msg.print_rds();
            break;
        }
            
        case 20: {
            PBITMessage pbit_msg = PBITMessage();
            cout << "Creating PBITMessage...\n";
            pbit_msg.bufferCommData();
            pbit_msg.create_msg();
            pbit_msg.calculate_crc();
            cout << "Sending PBITMessage...\n";
            serial.send(pbit_msg.buffer, pbit_msg.length);
            uint8_t* recv_msg = serial.receive();
            cout << "Receiving...\n\n";
            cout << "Response Message:\n";
            int len = recv_msg[0];
            if (len == 205) {
                cout << "No response.\n" << endl;
                break;
            }
            memcpy(&pbit_msg.rds, recv_msg, len);
            pbit_msg.print_rds();
            break;
        }
            
        default:
            break;
        }
    }
}


